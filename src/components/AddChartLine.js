...
export const AddChartLine = ({n_id_d}) => {
  // console.log(n_id_d)
  ...
  const [TypeChart, setTypeChart] = useState("line");//guarda el tipo de chart
  ...
  useEffect(() => {
    ...
    let elDato = sessionStorage.getItem(`DD_${n_id_d}`)
    if(elDato!==null){
  ...
  useEffect(() => {
    // console.log(MedData)
    if(MedData?.DATA){
      sessionStorage.setItem(`DD_${n_id_d}`,JSON.stringify(MedData))
    }
    return () => {
